...
class HunyuanVideoAdapter(BaseAdapter):
    """Adapter for HunyuanVideo (dual-stream, with optional true CFG)."""

    def cacheable_modules(self, pipeline: HunyuanVideoPipeline) -> dict[str, nn.Module]:
        """Returns a dictionary with all cacheable modules in HunyuanVideo."""
        mp = {}
        model: HunyuanVideoTransformer3DModel = pipeline.transformer
    ...
    def step_fn(self, pipeline: HunyuanVideoPipeline) -> Callable[[], int]:
    ...
    def context_fn(self, pipeline: HunyuanVideoPipeline) -> Callable[[], str | None]:
